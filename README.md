...
- Landing pages de campanha ainda pendentes: `defesa-execucao-fiscal` (só a versão original, `-2` e `-3` não migram), `consultoria-tributaria-empresarial`, `consultoria-juridica` (destino já definido). `planejamento-tributario-area-saude` e `recuperacao-credito-tributario` já publicadas (15/08/2026), fora do menu principal, com compliance auditado; refinamento de copy/CTA fica para quando campanhas específicas de Ads forem estruturadas
